...
// NOTE: userProfilePlugin is NOT used because profile creation happens
// via a separate authenticated endpoint (/api/user/profile/create) after signup
// This allows the frontend to send profile data separately from auth credentials
// Better Auth manages its own database connection via Kysely adapter

/**
 * Better Auth instance with optimized configuration
 */
const auth = betterAuth({
  // CRITICAL FIX: Better Auth requires a Kysely adapter or connection string
  // Passing raw Pool instance causes session creation to fail silently
  // Using Kysely adapter for proper PostgreSQL integration
  database: {
    provider: 'postgres',
  ...
  advanced: {
    ...
    // CRITICAL FIX: Do NOT set domain for cross-domain cookies on different origins
    // Setting domain: '.vercel.app' causes browsers to reject cookies
    // Instead, use host-only cookies with credentials: 'include' and sameSite: 'none'
    crossSubDomainCookies: {
      enabled: false  // FIXED: Disabled to allow host-only cookies for CORS
    },
  ...
  // Custom plugins
  // NOTE: No plugins needed - profile creation handled by /api/user/profile/create endpoint
  plugins: []
});
