...
const auth = betterAuth({
  ...
  // CRITICAL: Hooks for auto-creating user profile on signup
  hooks: {
    after: [
      {
        matcher: (context) => {
          // Trigger on sign-up endpoints
          return context.path === '/sign-up/email' && context.method === 'POST';
        },
        handler: async (context) => {
          // Only proceed if signup was successful and we have user data
          if (context.returned && context.returned.user) {
            const userId = context.returned.user.id;
            const { skillLevel, softwareBackground, hardwareBackground, learningGoal } = context.body || {};

            try {
              // Auto-create user profile with data from signup form
              await query(`
              ...
              logger.info('User profile auto-created on signup', {
                userId,
                hasSkillLevel: !!skillLevel,
                hasSoftwareBackground: !!softwareBackground,
                hasHardwareBackground: !!hardwareBackground,
                hasLearningGoal: !!learningGoal
              });
            } catch (error) {
              ...
              // Don't throw - we don't want to fail signup if profile creation fails
            }
          }
